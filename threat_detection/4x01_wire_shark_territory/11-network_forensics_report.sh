...
#
# MedDefense Health Systems — Wireshark Territory (Task 11: The Network
# Forensics Report)
#
# Purpose
#   Generates 11-network_forensics_report.md, the synthesis document for
#   this module: every finding from Tasks 0-8 (baseline, phishing click,
#   C2 beaconing, VPN pivot, lateral movement, DNS exfiltration, kill-chain
#   reconstruction, detection engineering, evidence cross-check) brought
#   together into one report for incident-response leadership.
#
#   This script writes a fixed document, not a live PCAP analysis — it
#   has no tshark commands of its own. Every factual claim in the
#   generated report (timestamps, byte counts, packet counts, decoded
#   markers, ASN/geolocation) is a value this module already confirmed by
#   running tshark against the real PCAPs in Tasks 0, 1, 3, 4, 5 and 6, or
#   an explicitly-labeled analytical inference/recommendation, never an
#   invented figure. Where a fact could not be independently confirmed in
#   this report (for example, the original 4x00 phishing email's exact
#   subject line and send timestamp, which this module's own PCAPs do not
#   record), the report says so rather than restating an unverified figure
#   as established fact.
#
# Usage
#   ./11-network_forensics_report.sh
...
OUTFILE="11-network_forensics_report.md"
