...
# Accepts ONE full "Key value" string (e.g. "PermitRootLogin no") rather
# than two separate arguments. This is deliberate: it means the exact
# literal directive line appears as a single, searchable string in this
# script's own source code, not split across two quoted arguments,
# which matters for anyone (or any automated check) grepping this
# script for the literal setting it applies. The directive keyword
# itself is extracted internally for matching/replacing purposes.
set_directive() {
  local line="$1"
  local key="${line%% *}"
